        ...
        <section className="dictionary-layout" id="dictionary">
          ...
          <article className="entry-card">
            ...
            {hasDetail ? <>
              ...
              {activeEntry.senses?.length > 1 && <section className="senses-section"><div className="section-heading"><span>更多释义</span><span className="line" /></div><ol>{activeEntry.senses.slice(1).map((sense, index) => <li key={`${sense.type}-${sense.definition}`}><span>{String(index + 2).padStart(2, '0')} · {sense.type}</span><p>{sense.definition}</p></li>)}</ol></section>}
